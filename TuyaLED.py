...
@lru_cache
def timeonoff(date):
    dt = datetime(date[0], date[1], date[2], date[3])
    latitude = 51.21
    longitude = 21.01
    ...
    if (date.month == 12 and date.day == 30) or (date.month == 12 and date.day == 31) or (date.month == 1 and date.day == 1) or (date.month == 1 and date.day == 2):
...
try:
    ...
    d.set_colour(0, 0, 0, nowait=False)
    print('EXIT')
    time.sleep(2)
